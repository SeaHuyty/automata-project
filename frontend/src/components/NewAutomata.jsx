...
export default function NewAutomata() {
    ...
    const handleSave = async () => {
        ...
        if (!startState) {
            toast.error("Please select a start state");
            return;
        }
        
        if (finalStates.length === 0) {
            toast.error("Please select at least one final state");
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                toast.success("Automaton saved successfully!");
                navigate("/");
            } else {
                toast.error("Failed to save automaton: " + result.error);
            }
        } catch (err) {
            console.error("Error saving automaton:", err);
            toast.error("Failed to save automaton. Please try again.");
        }
    };
        return (
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-8">
                    ...
                    <div className="mb-6 sm:mb-8 animate-fade-in">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
                            <div>
                                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-800 mb-2">Create New Automaton</h1>
                                <p className="text-sm sm:text-base text-gray-600">Define your DFA or NFA with complete specifications</p>
                            </div>
                            <button
                                onClick={() => navigate('/')}
                        ...
                        <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md border border-gray-100 mb-4 sm:mb-6">
                            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-end">
                                ...
                                <button
                                    onClick={handleSave}
                                    className="px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-[#1a365d] to-[#2d4a7c] text-white rounded-xl font-semibold hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2 text-sm sm:text-base"
                                >
